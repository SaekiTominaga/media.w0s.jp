...
await test('type undefined', async () => {
	...
	assert.equal(await res.text(), 'The `type` parameter is required');
});
...
await test('type multi', async () => {
	...
	assert.equal(res.status, 400);
	assert.equal(await res.text(), 'The `type` parameter can only be singular');
});
...
await test('type invalid value', async () => {
	...
	assert.equal(res.status, 400);
	assert.equal(await res.text(), 'The value of the `type` parameter is not an accepted string');
});
...
await test('w multi', async () => {
	...
	assert.equal(res.status, 400);
	assert.equal(await res.text(), 'The `w` parameter can only be singular');
});
...
await test('w string', async () => {
	...
	assert.equal(res.status, 400);
	assert.equal(await res.text(), 'The value of the `w` parameter must be a positive integer');
});
...
await test('w min', async () => {
	...
	assert.equal(res.status, 400);
	assert.equal(await res.text(), 'The value of the `w` parameter must be between 1 and 9999');
});
...
await test('h multi', async () => {
	...
	assert.equal(res.status, 400);
	assert.equal(await res.text(), 'The `h` parameter can only be singular');
});
...
await test('h string', async () => {
	...
	assert.equal(res.status, 400);
	assert.equal(await res.text(), 'The value of the `h` parameter must be a positive integer');
});
...
await test('h min', async () => {
	...
	assert.equal(res.status, 400);
	assert.equal(await res.text(), 'The value of the `h` parameter must be between 1 and 9999');
});
...
await test('quality multi', async () => {
	...
	assert.equal(res.status, 400);
	assert.equal(await res.text(), 'The `quality` parameter can only be singular');
});
